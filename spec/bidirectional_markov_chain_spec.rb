...
describe Markov::BidirectionalMarkovChain do
  ...
  context "new" do
    it "builds a chain with an order (# of current/past states used to predict the future) of 1 or greater" do
      Markov::BidirectionalMarkovChain.new(order=1, lookahead=3, num_states=5).should be_an_instance_of Markov::BidirectionalMarkovChain
    end
    it "raises an error for an order of 0 or lower" do
      expect{ Markov::BidirectionalMarkovChain.new(order=0, lookahead=2, num_states=5) }.to raise_error(ArgumentError)
    end
    it "builds a chain with two or more states" do
      Markov::BidirectionalMarkovChain.new(order=1, lookahead=2, num_states=2).should be_an_instance_of Markov::BidirectionalMarkovChain
    end
    it "raises an error for fewer than 2 states" do
      expect{ Markov::BidirectionalMarkovChain.new(order=1, lookahead=1, num_states=1) }.to raise_error(ArgumentError)
    end
    it "raises an error if not looking ahead 1 or more steps" do
      expect{ Markov::BidirectionalMarkovChain.new(order=1, lookahead=0, num_states=2) }.to raise_error(ArgumentError)
    end
  end
  ...
  context "steps_left" do
    ...
    it "returns nil if more steps are left than the lookahead" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=2, num_states=2)
      mc.transition(state=1, steps_left=4)
      mc.steps_left.should be_nil
    end
    it "returns the number of steps left if 0..(lookahead-1) steps left" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=4, num_states=2)
  ...
  context "order" do
    it "returns the number of historical states the chain uses to predict future states" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=2, num_states=3)
      mc.order.should == 1
    end
  end
  ...
  context "load" do
    it "loads the markov chain to a file" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=1, num_states=20)
  ...
  context "observe" do
    ...
    it "raises an error if steps_left is !nil and the observed steps_left is not exactly one less" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=5, num_states=2)
      mc.transition(state=1, steps_left=3)
      expect{ mc.observe(state=1, steps_left=1) }.to raise_error(ArgumentError)
    end
  ...
  context "get_expectations" do
    ...
    it "returns a random variable that only chooses states observed (higher order)" do
      mc = Markov::BidirectionalMarkovChain.new(order=2, lookahead=1, num_states=5)
    ...
    it "returns a random variable that only chooses states observed with the same steps remaining" do
      mc = Markov::BidirectionalMarkovChain.new(order=1, lookahead=1, num_states=5)
      mc.observe(   state=1, steps_left=3)
      mc.transition(state=1, steps_left=3)
      mc.observe(   state=3, steps_left=2)
      mc.transition(state=3, steps_left=2)
