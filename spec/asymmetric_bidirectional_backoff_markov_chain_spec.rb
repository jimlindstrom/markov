...
describe Markov::AsymmetricBidirectionalBackoffMarkovChain do
  ...
  context "new" do
    it "builds a chain with an order (# of current/past states used to predict the future) of 2 or greater" do
      Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=5, num_outcomes=3).should be_an_instance_of Markov::AsymmetricBidirectionalBackoffMarkovChain
    end
    it "raises an error for an order of 0 or lower" do
      expect{ Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=1, lookahead=1, num_states=5, num_outcomes=3) }.to raise_error(ArgumentError)
    end
    it "builds a chain with two or more states" do
      Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=2, num_outcomes=3).should be_an_instance_of Markov::AsymmetricBidirectionalBackoffMarkovChain
    end
    it "raises an error for fewer than 2 states" do
      expect{ Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=1, num_outcomes=3) }.to raise_error(ArgumentError)
    end
    it "builds a chain with two or more outcomes" do
      Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=2, num_outcomes=2).should be_an_instance_of Markov::AsymmetricBidirectionalBackoffMarkovChain
    end
    it "raises an error for fewer than 2 outcomes" do
      expect{ Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=2, num_outcomes=1) }.to raise_error(ArgumentError)
    end
    it "builds a chain with lookahead of one or more" do
      Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=2, num_outcomes=3).should be_an_instance_of Markov::AsymmetricBidirectionalBackoffMarkovChain
    end
    it "raises an error for less than 1 lookahead" do
      expect{ Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=0, num_states=3, num_outcomes=3) }.to raise_error(ArgumentError)
    end
  end

  context "current_state" do
    it "returns nil if in the initial state" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=2, num_outcomes=3)
  ...
  context "steps_left" do
    ...
    it "returns nil if more steps are left than the lookahead" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=2, num_states=2, num_outcomes=5)
      mc.transition(state=1, steps_left=4)
      mc.steps_left.should be_nil
    end
    it "returns the number of steps left if 0..(lookahead-1) steps left" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=4, num_states=2, num_outcomes=5)
  ...
  context "order" do
    it "returns the number of historical states the chain uses to predict future states" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=3, num_outcomes=3)
      mc.order.should == 2
    end
  end
  ...
  context "load" do
    it "loads the markov chain to a file" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=20, num_outcomes=4)
    ...
    it "restores the lookahead proprely" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=7, num_states=20, num_outcomes=4)
  ...
  context "get_expectations" do
    ...
    it "isn't surprised by repeated substrings in a long string" do
      mc = Markov::AsymmetricBidirectionalBackoffMarkovChain.new(order=2, lookahead=1, num_states=100, num_outcomes=90)
